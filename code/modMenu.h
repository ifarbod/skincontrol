...
enum Control
{
    ...
    // MenuAxisX = 195,					// A? + D + Controller RB? + Controller LB + Controller Left Left? + Controller Left
    // Right  MenuAxisY = 196,					// W? + S + Controller Left Up? + Controller Left Down  MenuRAxisX = 197,
    // // [? + ] + Controller Right Left? + Controller Right Right  MenuRAxisY = 198,					// MouseWheel Up? +
    // MouseWheel Down + Controller Right Up? + Controller Right Down  Escape = 200,						// Escape
    ...
    // JumpSpeedUpDown = 334,			// PageUp? + PageDown + MouseWheel Up? + MouseWheel Down + Controller Right Up? +
    // Controller Right Down  JumpSpeedUp = 335,				// PageUp + MouseWheel Up
...
// std::string caption  	The text to render. Passing an empty string will cause unintended behavior. Use a space " "
// instead. float rectWidth      	The width of the rectangle in pixels assuming the screen is 1280x720. float
// vertPadding    	How far the rectangle should extend beyond the text vertically. Measured in pixels assuming the
// screen is 1280x720.
// float rectTop			    The distance of the top of the rectangle from the top of the screen assuming the screen is
// 1280x720. float rectLeft       	    The distance of the left of the rectangle from the left of the screen assuming
// the screen is 1280x720. float textLeft       	    How far the text should be from the left side of the rectangle.
// Measured in pixels assuming the screen is 1280x720.
...
// float scale          	        The desired size/scale for the text. It appears to be preserved across external
// resolutions.
//                      	            Every 0.05 of scale corresponds to about 2 pixels of vertical resolution of the
//                      internal 720. Defaults to 0.35.
...
void drawRectWithText(std::string caption, float rectWidth, float vertPadding, float rectTop, float rectLeft,
    float textLeft, bool center = false, Color textColor = 0xFFFFFFFF, float scale = 0.35, int font = 0,
    Color backColor = 0x0000009B);

void drawRectWithText(std::string caption, float rectWidth, float vertPadding, float rectTop, float rectLeft,
    Color textColor = 0xFFFFFFFF, Color backColor = 0x0000009B);
...
void draw_menu_line(std::string caption, float rectWidth, float vertPadding, float rectTop, float rectLeft,
    float textLeft, bool active, bool title, bool rescaleText = true);
