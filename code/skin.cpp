...
void setSkinComponentOfID(PedSkin& skin, Ped pedID, int id)
{
    if (id < firstProp)
        PED::SET_PED_COMPONENT_VARIATION(
            pedID, id, skin.drawable[id], skin.texture[id], PED::GET_PED_PALETTE_VARIATION(pedID, id));
...
void renderPlayerSkin(
    PedSkin& skin, WeaponData& currentWeaponData, int index, bool newModel, bool fromSave, bool& nonDefaultSkin)
{
    if (STREAMING::IS_MODEL_VALID(skin.model) && STREAMING::IS_MODEL_IN_CDIMAGE(skin.model))
    {
        if (newModel)
        {
            ...
            while (!STREAMING::HAS_MODEL_LOADED(skin.model))
                WAIT(0);
